...
const STORES = {
	...
	scripts: {
		...
		value: {
			ping: `"Pong"`,
			uptime: `((Date.now() - startedDate) / 1000) + "s"`
		}
	}
};
...
function executeKey(line) {
	...
	if(typeof object !== "string") {
		return `<i>code is not a string</i>\n`;
	}

	return escapeHTML(JSON.stringify(eval(object)) || "undefined") + "\n";
}
...
function listKeys() {
	...
	for(const store in STORES) {
		result += `<i>Store</i> ${escapeHTML(store)}${STORES[store].readable ? "" : " <i>(unreadable)</i>"}${STORES[store].writable ? "" : " <i>(unwritable)</i>"}${STORES[store].executable ? " <i>(executable)</i>" : ""}`;
